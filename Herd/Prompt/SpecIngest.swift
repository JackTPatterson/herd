...
/// Herd's own shape. The source is the MIT-licensed Fig completion specs
...
enum SpecIngest {
    ...
    private nonisolated static func ingest(into destination: String, progress: (String) -> Void) throws -> Result {
        ...
        let work = manager.temporaryDirectory.appendingPathComponent("herd-specs-" + UUID().uuidString)
    ...
    private nonisolated static func notice(version: String) -> String {
        ...
        These files are generated by Herd from \(sourceName), version \(version),
        and converted into Herd's own format. The original licence is in
        `LICENSE-fig`; the upstream project is https://github.com/withfig/autocomplete.

        Herd fetches this corpus on request rather than bundling it, and adds its
        own sources on top: your history, this project's scripts and targets, and
        live values from cached generators.
        """
    }

    /// Imports each compiled spec and writes Herd's normalised JSON. Fig's
    /// generators are functions, so only their static shape survives; Herd
